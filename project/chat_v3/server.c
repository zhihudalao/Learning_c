...
void handleClientPrivateChatRequest(PDU *pdu, int sockfd)
{
	...
	Node *node = findNodeById(g_pList, pdu->uiTo);
	if (NULL == node)
	{
		PDU respdu;
		respdu.uiMsgType = MSG_TYPE_PRIVATE_CHAT_RESPOND;
		strcpy(respdu.caMsg, "数据发送失败：聊天对象不存在");
		write(sockfd, &respdu, sizeof(PDU));
		return;
	}
	if (node->sockfd <= 0)
	{
		PDU respdu;
		respdu.uiMsgType = MSG_TYPE_PRIVATE_CHAT_RESPOND;
		strcpy(respdu.caMsg, "数据发送失败：聊天对象不在线");
		write(sockfd, &respdu, sizeof(PDU));
		return;
	}	

	write(node->sockfd, pdu, sizeof(PDU));
}
